...
home = read_file(site_dir, "index.html", failures)
...
welcome = read_file(site_dir, File.join("essays", "welcome", "index.html"), failures)
...
if essays_index
  ...
  assert_contains(essays_index, %r{class="essay-year-heading"[^>]*>2024</h2>}, "essays index missing year grouping", failures)
  assert_contains(essays_index, %r{class="essay-list-description muted"}, "essays index missing descriptions", failures)
  assert_contains(essays_index, %r{class="tag">Design</span>}, "essays index missing optional tags", failures)
end

if home
  assert_contains(home, %r{class="essay-list essay-list-compact"}, "home page missing shared recent-essay list", failures)
  latest_position = home.index(%q{href="/essays/social-fallback-demo/"})
  oldest_position = home.index(%q{href="/essays/technical/"})
  failures << "home recent essays are not reverse chronological" unless latest_position && oldest_position && latest_position < oldest_position
end
...
if rich_demo
  assert_contains(rich_demo, %r{<iframe src="https://www\.youtube-nocookie\.com/embed/M7lc1UVf-VE"}, "rich-content-demo missing YouTube player", failures)
  assert_contains(rich_demo, %r{<iframe src="https://player\.vimeo\.com/video/76979871"}, "rich-content-demo missing standalone Vimeo player", failures)
  assert_contains(rich_demo, %r{<span>\d+ min read</span>}, "rich-content-demo missing reading time", failures)
  assert_contains(rich_demo, %r{class="essay-pagination"}, "rich-content-demo missing essay navigation", failures)
  failures << "rich-content-demo contains an unprocessed video placeholder" if rich_demo.include?("data-video-src=")
end

if welcome
  assert_contains(welcome, %r{class="essay-tags"[^>]*aria-label="Topics"}, "welcome essay missing tags", failures)
  assert_contains(welcome, %r{rel="prev"}, "welcome essay missing older navigation", failures)
  assert_contains(welcome, %r{rel="next"}, "welcome essay missing newer navigation", failures)
end
